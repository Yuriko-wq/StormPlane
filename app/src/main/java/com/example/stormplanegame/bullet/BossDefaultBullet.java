...
import com.example.stormplanegame.plane.MyPlane;
import com.example.stormplanegame.plane.SmallPlane;
...
public class BossDefaultBullet extends EnemyBullet {
    ...
    public void logic() {
        if (object_y >= 0) {
            object_y -= speed;
            object_x += (float) (10*(Math.sin(object_y)));
        }
        else {
            isAlive = false;
        }
        ...
        if (object_y2 >= 0) {
            object_y2 -= speed;
            object_x2 += (float) (-10*(Math.sin(object_y2)));
        }
        else{
            isAlive2 = false;
        }
